...
main(int argc, char **argv)
{
	...
	Binit(&bin, 0, OREAD);
	Binit(&bout, 1, OWRITE);
	while ((ch = Bgetrune(&bin.Biobufhdr)) != Beof) {
		c = ch;
		switch (c) {
		case '\n':
			incr();
...
emit(char *s, int lineno)
{
	...
	if (*s) {
		...
		if (cline != lineno) {
			Bputc(&bout.Biobufhdr, ESC);
			Bputc(&bout.Biobufhdr, '9');
			cline++;
		}
		if (pcp)
			Bputc(&bout.Biobufhdr, '\r');
		pcp = 0;
		p = s;
		while (*p) {
			ncp = pcp;
