...
getspec(void)
{
	...
	left1flg = rightl = 0;
	readspec();
	Bprint(&tabout.Biobufhdr, ".rm");
	for (i = 0; i < ncol; i++)
		Bprint(&tabout.Biobufhdr, " %2s", reg(i, CRIGHT));
	...
	while (c = get1char()) {
		...
		case 'a':
			...
			if (c == 's' && style[icol-1][nclin] == 'a') {
				Bprint(&tabout.Biobufhdr, ".tm warning: can't span a-type cols, changed to l\n");
				style[icol-1][nclin] = 'l';
			}
			if (c == 's' && style[icol-1][nclin] == 'n') {
				Bprint(&tabout.Biobufhdr, ".tm warning: can't span n-type cols, changed to c\n");
		...
		case 'W':
		   ...
		   of different widths now ....
			if (*snp)
				{
				Bprint(&tabout.Biobufhdr, "Ignored second width specification");
				continue;
				}
