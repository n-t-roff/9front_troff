...
maktab(void)			/* define the tab stops of the table */
{
	...
	for (icol = 0; icol < ncol; icol++) {
		...
		Bprint(&tabout.Biobufhdr, ".nr %2s 0\n", reg(icol, CRIGHT));
		for (text = 0; text < 2; text++) {
			if (text)
				Bprint(&tabout.Biobufhdr, ".%2s\n.rm %2s\n", reg(icol, CRIGHT),
				    reg(icol, CRIGHT));
			for (ilin = 0; ilin < nlin; ilin++) {
				...
				case 'a':
					...
					if (s > 0 && s < 128 && text) {
						if (doubled[icol] == 0)
							Bprint(&tabout.Biobufhdr, ".nr %d 0\n.nr %d 0\n",
							    S1, S2);
						doubled[icol] = 1;
				...
				case 'n':
					if (table[ilin][icol].rcol != 0) {
						...
						if (real(ss = table[ilin][icol].col) && !vspen(ss)) {
							...
							if (tx(s) != text) 
								continue;
							Bprint(&tabout.Biobufhdr, ".nr %d ", TMP);
						...
						if (text == 0 && real(ss = table[ilin][icol].rcol) && !vspen(ss) && !barent(ss)) {
							Bprint(&tabout.Biobufhdr, ".nr %d \\w%c%s%c\n",
							    TMP, F1, ss, F1);
							Bprint(&tabout.Biobufhdr, ".if \\n(%d<\\n(%d .nr %d \\n(%d\n", S2, TMP, S2,
							     TMP);
						}
		...
		if (cll[icol][0]) {
			Bprint(&tabout.Biobufhdr, ".nr %d %sn\n", TMP, cll[icol]);
			Bprint(&tabout.Biobufhdr, ".if \\n(%2s<\\n(%d .nr %2s \\n(%d\n",
			    reg(icol, CRIGHT), TMP, reg(icol, CRIGHT), TMP);
		}
		for (ilin = 0; ilin < nlin; ilin++)
			if (k = lspan(ilin, icol)) {
				...
				for (ik = k; ik >= 0; ik--) {
					...
					if (!expflg && ik > 0) 
						Bprint(&tabout.Biobufhdr, "-%dn", sep[icol-ik]);
				}
				Bprint(&tabout.Biobufhdr, "\n");
				Bprint(&tabout.Biobufhdr, ".if \\n(%d>0 .nr %d \\n(%d/%d\n", TMP,
				      TMP, TMP, k);
	...
	if (evenflg) {
		...
		for (icol = 0; icol < ncol; icol++) {
			if (evenup[icol] == 0)
				...
				continue;
			if (doubled[icol])
				Bprint(&tabout.Biobufhdr, ".nr %2s (100*\\n(%2s/\\n(%2s)*\\n(%d/100\n",
	...
	for (tsep = icol = 0; icol < ncol; icol++)
		tsep += sep[icol];
	if (expflg) {
		Bprint(&tabout.Biobufhdr, ".nr %d 0", TMP);
		for (icol = 0; icol < ncol; icol++)
			Bprint(&tabout.Biobufhdr, "+\\n(%2s", reg(icol, CRIGHT));
		Bprint(&tabout.Biobufhdr, "\n");
		Bprint(&tabout.Biobufhdr, ".nr %d \\n(.l-\\n(%d\n", TMP, TMP);
		...
		Bprint(&tabout.Biobufhdr, ".nr %d 1n\n", TMP);
	Bprint(&tabout.Biobufhdr, ".nr %2s 0\n", reg(-1, CRIGHT));
	...
	for (icol = 0; icol < ncol; icol++) {
		...
		if (doubled[icol]) {
			...
			Bprint(&tabout.Biobufhdr, ".nr %2s +\\n(%2s\n", reg(icol, CMID),
			    reg(icol, CLEFT));
			/*  Bprint(&tabout.Biobufhdr, ".if n .if \\n(%s%%24>0 .nr %s +12u\n",reg(icol,CMID), reg(icol,CMID)); */
		}
		tsep = sep[icol] * 2;
	}
	if (rightl)
		Bprint(&tabout.Biobufhdr, ".nr %s (\\n(%s+\\n(%s)/2\n", reg(ncol - 1, CRIGHT),
		      reg(ncol - 1, CLEFT), reg(ncol - 2, CRIGHT));
	Bprint(&tabout.Biobufhdr, ".nr TW \\n(%2s\n", reg(ncol - 1, CRIGHT));
	tsep = sep[ncol-1];
	if (boxflg || allflg || dboxflg)
		Bprint(&tabout.Biobufhdr, ".nr TW +((%d*\\n(%d)/2)\n", tsep, TMP);
	Bprint(&tabout.Biobufhdr,
...
{
	if (point(s)) {
		Bprint(&tabout.Biobufhdr, "\\w%c", F1);
		if (*fn > 0) 
			putfont(fn);
		if (*size) 
			putsize(size);
