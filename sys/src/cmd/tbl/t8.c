				...
				/* nl is line number for finding data   usually identical */
{
	...
	if (allh(i) && !pr1403) {
		...
		Bprint(&tabout.Biobufhdr, ".nr 35 \\n(.vu\n");
	} else
		Bprint(&tabout.Biobufhdr, ".nr 35 1m\n");
	if (chfont)
		Bprint(&tabout.Biobufhdr, ".nr %2d \\n(.f\n", S1);
	Bprint(&tabout.Biobufhdr, "\\&");
	vct = 0;
	for (c = 0; c < ncol; c++) {
		uphalf = 0;
...
/* code folded from here */
	Bprint(&tabout.Biobufhdr, "/1v*1v");
/* unfolding */
								Bprint(&tabout.Biobufhdr, "'");
								exvspen = 1;
							}
						}
					Bprint(&tabout.Biobufhdr, "%c%c", F1, F2);
					if (uphalf) 
						Bprint(&tabout.Biobufhdr, "\\u");
					puttext(ss, fn, size);
					if (uphalf) 
						Bprint(&tabout.Biobufhdr, "\\d");
					Bprint(&tabout.Biobufhdr, "%c", F1);
				}
				ss = table[nl][c].rcol;
				form = 1;
				break;
...
puttext(char *s, char *fn, char *size)
{
	if (point(s)) {
		putfont(fn);
		putsize(size);
		Bprint(&tabout.Biobufhdr, "%s", s);
		if (*fn > 0) 
			Bprint(&tabout.Biobufhdr, "\\f\\n(%2d", S1);
		if (size != 0) 
			putsize("0");
	}
}
...
funnies(int stl, int lin)
{
	...
	for (c = 0; c < ncol; c++) {
		...
		if (ss == 0) 
			continue;
		s = (int)(uintptr)ss;
		Bprint(&tabout.Biobufhdr, ".sp |\\n(##u-1v\n");
		Bprint(&tabout.Biobufhdr, ".nr %d ", SIND);
		ct = 0;
		...
		Bprint(&tabout.Biobufhdr, ".%c+\n", s);
		Bprint(&tabout.Biobufhdr, ".in -\\n(%du\n", SIND);
		if (*fn > 0) 
			putfont("P");
		Bprint(&tabout.Biobufhdr, ".mk %d\n", S2);
		Bprint(&tabout.Biobufhdr, ".if \\n(%d>\\n(%d .nr %d \\n(%d\n", S2, S1, S1, S2);
	}
	Bprint(&tabout.Biobufhdr, ".sp |\\n(%du\n", S1);
	for (c = dv = 0; c < ncol; c++) {
...
{
	if (fn && *fn)
		Bprint(&tabout.Biobufhdr,  fn[1] ? "\\f(%.2s" : "\\f%.2s",  fn);
}
