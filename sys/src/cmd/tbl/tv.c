...
drawvert(int start, int end, int c, int lwid)
{
	...
	for (ln = 0; ln < lwid; ln++) {
		...
		pos = 2 * ln - lwid + 1;
		if (pos != tp) 
			Bprint(&tabout.Biobufhdr, "\\h'%dp'", pos - tp);
		tp = pos;
		if (end < nlin) {
		...
		if (exb)
			Bprint(&tabout.Biobufhdr, "\\v'%s'", exb);
		if (epb)
			Bprint(&tabout.Biobufhdr, "\\v'%dp'", epb);
		...
		Bprint(&tabout.Biobufhdr, "\\L'|\\n(#%cu-%s", linestop[start] + 'a' - 1,
		      vm == 'v' ? "1v" : "\\n(35u");
		if (ext)
			Bprint(&tabout.Biobufhdr, "-(%s)", ext);
		if (exb)
			Bprint(&tabout.Biobufhdr, "-(%s)", exb);
		pos = ept - epb;
		if (pos)
			Bprint(&tabout.Biobufhdr, "%s%dp", pos >= 0 ? "+" : "", pos);
		/* the string #d is either "nl" or ".d" depending
	   on diversions; on GCOS not the same */
		Bprint(&tabout.Biobufhdr, "'\\s0\\v'\\n(\\*(#du-\\n(#%cu+%s",
		     linestop[start] + 'a' - 1, vm == 'v' ? "1v" : "\\n(35u");
		if (ext)
			Bprint(&tabout.Biobufhdr, "+%s", ext);
		if (ept)
			Bprint(&tabout.Biobufhdr, "%s%dp", (-ept) > 0 ? "+" : "", (-ept));
