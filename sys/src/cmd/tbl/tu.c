...
drawline(int i, int cl, int cr, int lintype, int noheight, int shortl)
{
	...
	if (!nodata)
		Bprint(&tabout.Biobufhdr, "\\v'-.5m'");
	for (ln = oldpos = 0; ln < lcount; ln++) {
		linpos = 2 * ln - lcount + 1;
		if (linpos != oldpos)
			Bprint(&tabout.Biobufhdr, "\\v'%dp'", linpos - oldpos);
		oldpos = linpos;
		if (shortl == 0) {
			tohcol(cl);
			if (lcount > 1) {
				...
				if (exhl[0])
					Bprint(&tabout.Biobufhdr, "\\h'%s'", exhl);
			} else if (lcount == 1) {
				switch (interv(i, cl)) {
				case TOP: 
				case BOT: 
		...
		if (linsize)
			Bprint(&tabout.Biobufhdr, "\\v'\\n(%dp/6u'", LSIZE);
		Bprint(&tabout.Biobufhdr, "\\s0");
	}
	if (oldpos != 0)
		Bprint(&tabout.Biobufhdr, "\\v'%dp'", -oldpos);
	if (!nodata)
		Bprint(&tabout.Biobufhdr, "\\v'+.5m'");
}
