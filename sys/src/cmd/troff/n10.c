...
/* god help us if we get out of sync. */
	...
	if (!is(devname9) && trace)
		ERROR "wrong terminal name: saw %s, wanted %s", cmd, devname9 WARN;
	for (;;) {
		fscanf(fp, "%s", cmd);
		if (is("charset"))
			break;
		...
		if (is("bset")) t.bset = atoi(opt);
			...
			ERROR "bad tab.%s file, %s %s", devname9, cmd, opt WARN;
	}
