...
void pile(int oct)
{
	...
	if (dbg) {
		printf(".\tS%d <- %d pile of:", yyval.token, type);
		for (i = p1; i < p2; i++)
			printf(" S%d", lp[i]);
	...
	printf(".ds %d \\v'%gm'\\h'%du*\\n(%du'\\\n", yyval.token, REL(ebase[yyval.token],ps), 
		type==RCOL ? 1 : 0, yyval.token);
	sb = 0;		/* sum of box hts */
	for (i = p2-1; i >= p1; i--) {
		bi = sb + ebase[lp[i]];
		switch (type) {
		...
		case COL:
			printf("\\v'%gm'\\h'\\n(%du-\\n(%du/2u'\\*(%d", 
				REL(-bi,ps), yyval.token, lp[i], lp[i]);
			printf("\\h'-\\n(%du-\\n(%du/2u'\\v'%gm'\\\n", 
				yyval.token, lp[i], REL(bi,ps));
			break;
		}
		sb += eht[lp[i]] + gap;
	}
	printf("\\v'%gm'\\h'%du*\\n(%du'\n", REL(-ebase[yyval.token],ps), 
		type!=RCOL ? 1 : 0, yyval.token);
	for (i = p1; i < p2; i++)
		sfree(lp[i]);
