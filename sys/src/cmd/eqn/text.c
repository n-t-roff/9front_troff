...
void text(int t, char *p1)	/* convert text string p1 of type t */
{
	...
	yyval.token = salloc();
	ebase[yyval.token] = 0;
	eht[yyval.token] = EM(1.0, ps);	/* ht in ems of orig size */
	lfont[yyval.token] = rfont[yyval.token] = ROM;
	lclass[yyval.token] = rclass[yyval.token] = OTHER;
	if (t == QTEXT) {
		...
		*csp = '\0';
		...
		rclass[yyval.token] = nclass;
	}
	dprintf(".\t%dtext: S%d <- %s; b=%g,h=%g,lf=%c,rf=%c,ps=%d\n",
		t, yyval.token, p, ebase[yyval.token], eht[yyval.token], lfont[yyval.token], rfont[yyval.token], ps);
	printf(".ds %d \"%s\n", yyval.token, p);
}
