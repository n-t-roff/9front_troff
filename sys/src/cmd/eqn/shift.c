...
void bshiftb(int p1, int dir, int p2)
{
	...
	if (dir == SUB) {
		...
		ebase[yyval.token] = shval + b2;
		eht[yyval.token] = max(h1-b1+shval+b2, h2);
		...
		eht[yyval.token] = h1 + max(0, h2 - (1-Supshift)*(h1-b1));
		if (rclass[p1] == ILETF)
			n = 4;
	...
	dprintf(".\tS%d <- %d shift %g %d; b=%g, h=%g, ps=%d, subps=%d\n", 
		yyval.token, p1, shval, p2, ebase[yyval.token], eht[yyval.token], ps, subps);
	sh2 = Sub2space;	/* was Sub2space; */
	printf(".as %d \\v'%gm'%s%s\\*(%d%s%s\\v'%gm'\n", 
		yyval.token, REL(shval,ps), DPS(ps,subps), sh1, p2,
...
void shift2(int p1, int p2, int p3)
{
	...
	if (b1 > b2 + subsh) /* move little sub down */
		subsh += b1;
	eht[yyval.token] = max(subsh+b2-b1+h1, h2);
	supsh = -Supshift*(h1-b1) - b3;
	d2 = EM(Subbase, subps);
	if (h3 < (1-Supshift)*(h1-b1))
		supsh = -(h1-b1) + (h3-b3) - d2;
	ebase[yyval.token] = subsh + b2 - b1;
	eht[yyval.token] = h1 + subsh+b2-b1 + max(0, h3-(1-Supshift)*(h1-b1));
	dprintf(".\tS%d <- %d sub %d sup %d, ps=%d, subps=%d, h=%g, b=%g\n",
		yyval.token, p1, p2, p3, ps, subps, eht[yyval.token], ebase[yyval.token]);
	if (rclass[p1] == ILETF)
		sh2 = "\\|\\|";
	else
		sh2 = SS2space;
	...
	*/
	...
	rclass[yyval.token] = rclass[p3];	/* was OTHER */
